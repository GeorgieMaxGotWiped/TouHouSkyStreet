...
class Bullet:
    ...
    def __init__(self, x, y, vx, vy, bullet_type=TYPE_CIRCLE, radius=3.0,
        ...
        self.split_spec = None       # 分裂配置：计时结束后在原地爆出子弹
    ...
    def update(self, dt):
        ...
        # 出界检查
        margin = 60
        if (self.x < -margin or self.x > cfg.BATTLE_AREA_WIDTH + margin or
    ...
    def _get_atlas_sprite(self):
        """按弹种从图集取裁剪贴图；未配置 / 半径过大 / 图集缺失时返回 None（回退图元绘制）。"""
        if self.is_player_bullet:
            return None
        ...
        tint = None
        if cfg.ENEMY_BULLET_SPRITE_TINT:
            tint = self.color
        key = (slot, width, angle, tuple(int(c) for c in tint) if tint else None)
        if key not in self._sprite_cache:
            self._sprite_cache[key] = bullet_atlas.get_sprite(slot, width, angle, tint)
