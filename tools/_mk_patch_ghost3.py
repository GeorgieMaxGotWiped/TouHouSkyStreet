# -*- coding: utf-8 -*-
# 临时脚本：插入加强后的四残影弹幕 / 片段长度 / 取消要塞段小怪（分三个补丁文件）
import io

path = "src/stages/stage6.py"
txt = io.open(path, encoding="utf-8").read()

NEW_B = '''def _ghost_spell_maxor(ghost, bullet_manager, player_x, player_y, t):
...
CTX = ("# 地狱火。残影本身打不到，打完这一小段就淡出。\n"
       "# ---------------------------------------------------------------------------\n"
       "def _ghost_spell(ghost, bullet_manager, player_x, player_y):\n")
assert txt.count(CTX) == 1
patch_i = ("*** Begin Patch\n*** Update File: %s\n@@\n" % path
           + " " + CTX.splitlines()[0] + "\n"
           + " " + CTX.splitlines()[1] + "\n"
           + "".join("+" + ln + "\n" for ln in NEW_B.splitlines())
           + " " + CTX.splitlines()[2] + "\n"
           + "*** End Patch\n")
io.open("tools/_patch_ghost_ins.txt", "w", encoding="utf-8", newline="\n").write(patch_i)

OLD_C = "GHOST_SPELL_FRAMES = 80   # 登场即起手的「告别弹」长度（帧，见 _ghost_spell）"
NEW_C = "GHOST_SPELL_FRAMES = 110  # 登场即起手的「告别弹」长度（帧，见 _ghost_spell）"
assert txt.count(OLD_C) == 1
io.open("tools/_patch_ghost_len.txt", "w", encoding="utf-8", newline="\n").write(
    "*** Begin Patch\n*** Update File: %s\n@@\n-%s\n+%s\n*** End Patch\n" % (path, OLD_C, NEW_C))

head = "        # 后半段：凋零要塞（66 ~ 100s）：敌人减少、场面变大"
tail = "        for t, wave in fortress_waves:\n            em.add_timed_wave(t, wave)\n"
i0 = txt.index(head)
i1 = txt.index(tail) + len(tail)
OLD_A = txt[i0:i1]
assert OLD_A.count("fortress_waves") == 2
NEW_A = """        # 后半段：凋零要塞（66 ~ 100s）：这一段不再生成小怪 —— 王之门徒的四位残影
...
"""
io.open("tools/_patch_ghost_waves.txt", "w", encoding="utf-8", newline="\n").write(
    "*** Begin Patch\n*** Update File: %s\n@@\n" % path
    + "".join("-" + ln + "\n" for ln in OLD_A.splitlines())
    + "".join("+" + ln + "\n" for ln in NEW_A.splitlines())
    + "*** End Patch\n")
print("3 patches written")
