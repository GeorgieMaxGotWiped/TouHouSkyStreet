# -*- coding: utf-8 -*-
# 临时脚本：生成「残影段不再生成小怪」+「四残影火力加强」的 apply_patch 文本
import io
...
OLD_A = txt[i0:i1]
assert OLD_A.count("fortress_waves") == 2, OLD_A
...
OLD_B = txt[g0:g1]
assert "def _ghost_spell_necron(" in OLD_B and "def _ghost_spell(" not in OLD_B.replace("def _ghost_spell_necron(", "")
...
for old, new in ((OLD_A, NEW_A), (OLD_B, NEW_B), (OLD_C, NEW_C)):
    assert txt.count(old) == 1, "anchor not unique (%d)" % txt.count(old)
    hunks.append("@@\n" + "".join("-" + ln + "\n" for ln in old.splitlines())
                 + "".join("+" + ln + "\n" for ln in new.splitlines()))

patch = "*** Begin Patch\n*** Update File: %s\n" % path + "".join(hunks) + "*** End Patch\n"
io.open("tools/_patch_ghost2.txt", "w", encoding="utf-8", newline="\n").write(patch)
