# -*- coding: utf-8 -*-
# 临时脚本：给 RELEASE_NOTES.md 追加两条六面道中改动说明
import io

path = "RELEASE_NOTES.md"
txt = io.open(path, encoding="utf-8").read()

ANCHOR = ("  - 影响：同行波次的敌人不再左右错开，压迫感从「蛇形逼近」变成「整列压下来」；"
          "实测在场时间不变（Husk 8.3s / Miner 15.5s / Knight 13.8s，均由下缘离场）\n")
assert txt.count(ANCHOR) == 1, "anchor not found/unique"

ADD = """
- 六面道中的凋零守卫 / 凋零矿工改为直接在画面内上部出现，凋零游魂变快变多（`src/stages/stage6.py`）：守卫与矿工原先从战斗区外（y=-24~-90）落下，先在框外走一段才进画面；现在按 `_top_spawn()` 把原来的区外高度换算成画面内落点（最低一档 y=220，原高度每高 1px 落点高 2.4px，先后关系与原版一致），入场即已站在战斗区上部（落点 220~330）；凋零游魂仍从区域外落下
...
patch = ("*** Begin Patch\n*** Update File: %s\n@@\n" % path
         ...
         + "*** End Patch\n")
io.open("tools/_patch_notes.txt", "w", encoding="utf-8", newline="\n").write(patch)
print("ok")
