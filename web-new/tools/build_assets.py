# -*- coding: utf-8 -*-
"""为新官网（web-new/）生成位图素材与数据。

用法（在项目根目录执行）：
    python web-new/tools/build_assets.py            # 全量
    python web-new/tools/build_assets.py shots      # 只重跑某一步
...
本脚本只读取仓库里的原始素材，并只写入 web-new/ 目录，不改动旧站（web/）与游戏本体。
"""
import json
import os
import sys
...
WEB_NEW = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ROOT = os.path.dirname(WEB_NEW)
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

ASSETS = os.path.join(WEB_NEW, "assets")
...
def _report(label, out_path, size, src_path):
    rel = os.path.relpath(out_path, WEB_NEW).replace("\\", "/")
...
def build_data():
    ...
    out = os.path.join(WEB_NEW, "data", "items.json")
...
def main():
    ...
    for name in wanted:
        ...
        if build is None:
            ...
            continue
        build()
    print("\n完成。产物位于 web-new/assets 与 web-new/data。")
