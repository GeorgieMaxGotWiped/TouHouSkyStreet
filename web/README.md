...
> 本目录就是站点根：`.github/workflows/pages.yml` 把 `web/` 打包发布到 GitHub Pages
> （首次使用需在仓库 Settings → Pages 把 Source 设为 "GitHub Actions"）。上一版官网
> 的画廊 / OST 两页与 `data/gallery.json` 已随改版下线，人物图并到人物页、曲目并到音乐室。
...
- 或手动：`python web/serve.py`（默认 8100 端口），浏览器访问 `http://127.0.0.1:8100`。
...
| 文件 | 来源 |
| --- | --- |
| `data/items.json` | `python web/tools/build_assets.py data`（读 `src/systems/item_system.py`） |
| `assets/**` | `python web/tools/build_assets.py`（读 `assets/` 与 `previews/` 下的原始素材） |
...
python web/tools/build_assets.py title      # 只重做首页底图
python web/tools/build_assets.py shots      # 只重做实机截图
python web/tools/build_assets.py data       # 只重新导出物品数据
```

步骤名：`title` / `stage` / `chara` / `boss` / `item` / `shots` / `icon` / `data`。

Boss 立绘有两个套组（游戏设置里可切换，见 `src/engine/settings.py` 的 `BOSS_ART_SETS`）。
本站统一用「另一版」：由 `web/tools/build_assets.py` 顶部的 `BOSS_ART_SET` 决定，
舞台页与人物页共用同一批图。想换回「新版」就改这一个常量，再跑 `python web/tools/build_assets.py boss`。

立绘在构建期归一化过：`tools/build_assets.py` 的 `_fit_portrait()` 先把每张图的**墨迹**
（非透明像素）拉平到画布高度、底边贴齐画布底、左右居中，再按原来的 `max_h` 出图。原始素材的
画幅与取景并不统一（多数 1024×1536，少数偏方 1200×1200；弓手整张偏右 85px，石守卫身高只有
画布的 92%），不归一化就会出现「每行人物左右跳 + 个别明显偏小」。归一化之后，人物页自机 / Boss
同一个 `max-height`（桌面 600px，往下依次 460 / 400 / 340）= 同一个视觉高度。
**改 `max-height` 要连带改列宽**（`.chara-full .inner` 与 `.boss-row` 里 `min(600px, calc(100% - 520px))`
的两个数），列宽至少要放得下最宽的那张（1200×1200 → 600px 宽）。
